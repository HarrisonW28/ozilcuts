...
final class AppointmentAdjustmentRequestApproveController extends Controller
{
    public function __invoke(
        ...
        try {
            $updated = $adjustments->approve($pending, $user);
        } catch (RuntimeException $e) {
            return response()->json(['message' => $e->getMessage()], 422);
        }

        return response()->json([
            'appointment' => (new AppointmentResource($updated))->toArray($request),
            'request' => null,
        ]);
    }
}
